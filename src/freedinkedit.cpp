...
/*   lpDDSTwo->BltFast(0, 0, tiles[tileset], &tilerect[tileset], DDBLTFAST_NOCOLORKEY |DDBLTFAST_WAIT); */
  // GFX
  SDL_BlitSurface(gfx_tiles[tileset], NULL, GFX_background, NULL);
  cur_tileset = tileset;
...
//use screenmatch technology
	...
	log_debug("need to add it to the right");
		...
		if (g_map.loc[cur_map+1] > 0)
		{
        	editor_save_screen(g_map.loc[cur_map]);
          log_debug("loading right map");
			editor_load_screen(g_map.loc[cur_map+1]);
               int holdx = spr[1].x;
               int holdy = spr[1].y;
			   spr[1].x -= 600;
			sp_add();
			editor_save_screen(g_map.loc[cur_map+1]);
	...
	log_debug("need to add it to the bottom");
		...
		if (g_map.loc[cur_map+32] > 0)
		{
        	editor_save_screen(g_map.loc[cur_map]);
          log_debug("loading bottom ");
			editor_load_screen(g_map.loc[cur_map+32]);
               int holdx = spr[1].x;
               int holdy = spr[1].y;
			   spr[1].y -= 400;
			sp_add();
			editor_save_screen(g_map.loc[cur_map+32]);
			editor_load_screen(g_map.loc[cur_map]);
           spr[1].x = holdx;
		   spr[1].y = holdy;
		}
		...
		if (((g_editorMode == EDITOR_MODE_SCREEN_TILES) && (sjoy.button[EDITOR_ACTION_RETURN]))
			...
			tilesel:
				   ...
				   before scaling it.. */
				{
					...
					cool = cur_tile / 128;
					gfx_blit_stretch(gfx_tiles[cool+1], &src, GFX_background, &dst);
				}
