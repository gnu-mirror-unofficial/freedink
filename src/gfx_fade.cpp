...
void gfx_fade_init()
{
  ...
  cache = (short unsigned int**)malloc(32*sizeof(unsigned short*));
  ...
  for (i = 0; i < 32; i++)
	...
	{
	  /* RGB565 */
	  cache[i] = (short unsigned int*)malloc(65536*sizeof(unsigned short));
	  for (j = 0; j < 65536-1; j++)
	    {
	      Uint8 r, g, b;
		   ...
		   && screen->format->Gmask == 0x03E0))
	{
	  ...
	  cache[i] = (short unsigned int*)malloc(32768*sizeof(unsigned short));
	  for (j = 0; j < 32768-1; j++)
	    {
	      Uint8 r, g, b;
